...
def print_park(T1, file):
    if park:
        ...
        file.write('#\npark  = ' + "HA" + ' ' + '194821.45  -084724.7' + '  ' + mag + ' '
...
t_exp = 10.0
n_frames = 10
exp_wait = 0  # 30  # interval between frames
t_between_ser = 900  # 60 * 5 seconds dead time between series
# t_miz_ser = 3.6*60*60
...
if Filter is None or Filter == "":
    ...
    str_v_plan = f"{n_frames}x{t_exp}:{exp_wait}*{Filter} @"
...
for ser in range(0, series):
    ...
    for i in range(0, len(geo_list)):
        ...
        if geo_list[i].planed[ser] == 0:
            ...
            if (geo_list[i].geo.alt > ephem.degrees("10")) and (T1 < end_T) and (moon_sep > ephem.degrees("30")):
                if not geo_list[i].geo.eclipsed:
                    ha_s, dec_s = corr_ha_dec_s(ha, geo_list[i].geo.dec)
                    f.write(f"{geo_list[i].NORAD} = {flag} {ha_s}({ra_speed:3.2f})  {dec_s}({dec_speed:3.2f}) {mag} {str_v_plan}{T1_s}-{T2_s}\n")
                    ...
                    if found:
                        ...
                        f.write(f"{sat.NORAD} = {flag} {ha_s}({ra_speed:3.2f})  {dec_s}({dec_speed:3.2f}) {mag} {str_v_plan}{T1_s}-{T2_s}\n")
            ...
            if i == len(geo_list) - 1:
                ...
                for j in range(0, len(geo_list)):
                    ...
                    if (geo_list[j].planed[ser] == 0) and (not geo_list[j].geo.eclipsed) and \
                        ...
                        ra_speed, dec_speed = calc_geo_speed(geo=geo_list[i], site=Deren, date=Deren.date, flag=C)
                        f.write(f"{geo_list[j].NORAD} = {flag} {ha_s}({ra_speed:3.2f})  {dec_s}({dec_speed:3.2f}) {mag} {str_v_plan}{T1_s}-{T2_s}\n")
